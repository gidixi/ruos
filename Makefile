...
# Lua interpreter: third-party C, NOT a cargo crate. This explicit rule shadows
# the pattern rule above for lua.wasm. The wasm is a vendored prebuilt artifact
# (committed, see third_party/lua/) so when present make treats it as up-to-date
# and never rebuilds. If it's missing, point at the build script instead of
# letting `make iso` die with a confusing cargo error.
user-bin/lua.wasm:
	@if [ ! -f $@ ]; then \
		echo "ERROR: $@ missing — build it once with: third_party/lua/build-wasm.sh"; \
		echo "       then commit it: git add -f user-bin/lua.wasm"; \
		exit 1; \
	fi
...
run: iso $(DISK_IMG)
	qemu-system-x86_64 -machine q35 -cpu max -boot d -cdrom $(ISO) -serial stdio -m 2048 \
...
run-test: $(DISK_IMG)
	...
	@timeout 240 qemu-system-x86_64 -machine q35 -cpu max -boot d -cdrom $(TEST_ISO) -serial stdio -display none -no-reboot -m 2048 \
		-device qemu-xhci -device usb-kbd -netdev user,id=net0 -device $(NIC),netdev=net0 \
		-drive file=$(DISK_IMG),format=raw,if=none,id=disk0 \
		-device ahci,id=ahci -device ide-hd,drive=disk0,bus=ahci.0 \
...
run-test-usb:
	...
	@timeout 240 qemu-system-x86_64 -machine q35 -cpu max -m 2048 -no-reboot -display none -serial stdio \
...
run-exec-ap-test:
	...
	@timeout 90 qemu-system-x86_64 -machine q35 -cpu max -smp 4 -m 2048 -no-reboot -display none -serial stdio \
...
run-console-test: iso
	...
	@timeout 60 qemu-system-x86_64 -machine q35 -cpu max -boot d -cdrom $(ISO) -serial stdio -display none -no-reboot -m 2048 \
...
test-boot: limine $(USER_WASMS) $(WT_KCWASMS) kernel/src/wasm/wt/bringup.cwasm kernel/src/wasm/wt/reactor.cwasm kernel/src/wasm/wt/reactor_close.cwasm kernel/src/wasm/wt/egui_demo.cwasm kernel/src/wasm/wt/shell.cwasm $(INIT_SCRIPT) build/wtecho.cwasm build/about.cwasm build/files.cwasm build/terminal.cwasm build/system.cwasm build/notepad.cwasm build/tui.cwasm build/rtop.cwasm
	...
	@timeout 60 qemu-system-x86_64 -machine q35 -cpu max -m 2048 -no-reboot -display none -serial stdio \
