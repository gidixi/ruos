...
//! of `crate::mouse::MouseEvent` — so no Y negation. Byte3, when present, is the
//! wheel (i8, HID convention: positive = away from the user = scroll up — already
//! the `MouseEvent.wheel` convention, no negation).

use crate::mouse::MouseEvent;

/// Decode a USB HID boot-protocol mouse report into a `MouseEvent`. Byte3 (wheel)
/// is decoded when present; a short report decodes the missing fields as zero.
pub fn decode_boot_mouse(report: &[u8]) -> MouseEvent {
    let buttons = report.first().copied().unwrap_or(0);
    // Bytes 1/2/3 are i8 deltas; a short report leaves the missing fields at 0.
    let dx = report.get(1).map_or(0, |&b| b as i8 as i16);
    let dy = report.get(2).map_or(0, |&b| b as i8 as i16);
    let wheel = report.get(3).map_or(0, |&b| b as i8 as i16);
    MouseEvent {
        dx,
        ...
        wheel, // HID: positive = away from user = scroll up, no negation.
    }
}

/// Boot-check self-test: deterministic reports exercise button bits, i8
/// sign-extension on both axes, the no-Y-negation convention, and the wheel byte.
...
pub fn self_test() -> bool {
    ...
    if decode_boot_mouse(&[0x06, 0xFE, 0xFF])
        != (MouseEvent { dx: -2, dy: -1, left: false, right: true, middle: true, wheel: 0 })
    {
        return false;
    }
    // Short report: missing Y decodes as 0, not a panic.
    if decode_boot_mouse(&[0x01, 0x04])
        != (MouseEvent { dx: 4, dy: 0, left: true, right: false, middle: false, wheel: 0 })
    {
        return false;
    }
    // 4-byte report with wheel: 0x01 = scroll up (positive, no negation),
    // 0xFF = scroll down (i8 sign-extension).
    if decode_boot_mouse(&[0x00, 0x00, 0x00, 0x01]).wheel != 1 {
        return false;
    }
    if decode_boot_mouse(&[0x00, 0x00, 0x00, 0xFF]).wheel != -1 {
        return false;
    }
    true
}
