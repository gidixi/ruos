...
impl SigType {
    ...
    fn verify_ed25519(
        ...
        let l = match sshwire::write_ssh(&mut buf, msg) {
            Ok(l) => l,
            Err(e) => {
                log::warn!("RUOS verify_ed25519 write_ssh err: {:?}", e);
                return Err(e);
            }
        };
        let buf = &buf[..l];
        match k.verify(buf, &s) {
            Ok(()) => Ok(()),
            Err(e) => {
                log::warn!("RUOS verify_ed25519 FAIL msg_len={} err={:?}", l, e);
                Err(Error::BadSig)
            }
        }
    }
