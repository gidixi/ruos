...
/// appropriate so that `ptr + len` stays within the original allocation.
///
/// ruos: the TOP of the stack (`ptr + len`) must be aligned too — the stack
/// grows DOWN from the top, the switch asm and Wasmtime's frame-pointer
/// unwinder assume `top % 16 == 0` (on Unix mmap gives page alignment for
/// free; a heap allocation does not). Upstream only aligned the base: with a
/// 16*k size and an 8-mod-16 heap pointer every frame ended up misaligned and
/// the unwinder's `assert_fp_is_aligned` fired on the first trap inside a
/// fiber. Round `new_len` DOWN to the alignment so both ends are aligned.
fn align_ptr(ptr: *mut u8, len: usize, align: usize) -> (*mut u8, usize) {
    let ptr = ptr as usize;
    let aligned = (ptr + align - 1) & !(align - 1);
    let new_len = (len - (aligned - ptr)) & !(align - 1); // ruos: align top too
